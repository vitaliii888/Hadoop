...
class OtherRecordReader extends RecordReader<Text, DoubleWritable> {
    ...
    private Set<Integer> incl = new HashSet<>();
    ...
    public void initialize(InputSplit split, TaskAttemptContext context)
        ...
        if (!initialized) {
            ...
            int incl_num = (int)(total_num * (1.0-sparsity));
            //Integer total_num = new Double((end_line - start_line + 1) * ncolumns).intValue();
            //Integer incl_num = new Double(total_num * (1.0-sparsity)).intValue();
            while (incl.size() < incl_num) {
                incl.add(chunkrand.nextInt(total_num - 1));
            }
            initialized = true;
        }
    }
    ...
    public boolean nextKeyValue() throws IOException, InterruptedException {
        value = incl.contains(it) ? min + chunkrand.nextDouble() * (max - min) : 0.0;
        while (incl.contains(it) && value == 0.0)
